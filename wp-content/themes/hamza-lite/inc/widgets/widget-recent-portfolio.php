...
add_action( 'widgets_init', 'hamza_lite_register_recent_portfolio_widget' );

function hamza_lite_register_recent_portfolio_widget() {
...
class Hamza_Lite_Recent_Portfolio extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			__('8D : Hamza Lite Recent Portfolio', 'hamza-lite'),
			array(
				'description'	=> __( 'A widget that shows recent portfolio with selected category', 'hamza-lite' )
			)
		);
	}
	...
	public function widget( $hamza_lite_args, $hamza_lite_instance ) {
        ...
        if($hamza_lite_recent_qry->have_posts()){
            ...
            while($hamza_lite_recent_qry->have_posts()){
                ...
                $hamza_lite_img_url = wp_get_attachment_image_src( get_post_thumbnail_id(), 'hamza-lite-portfolio-thumbnail', true );
            ?>
            <div class="hamza_lite-recent-rightdivs clearfix">
                ...
                <?php }else{ ?>
                <div class="hamza_lite-recent-rightbar-img">
                    <a href="<?php the_permalink(); ?>"><img src="<?php echo esc_url(get_template_directory_uri().'/images/image77by54.jpg'); ?>" alt="<?php the_title();?>" /></a>
                </div>
                <?php } ?>
            </div>            
            <?php
