...
function hamza_lite_header_style() {
	$hamza_lite_header_text_color = get_header_textcolor();

	// If no custom options for text are set, let's bail
	// get_header_textcolor() options: HEADER_TEXTCOLOR is default, hide text (returns 'blank') or any hex value
	if ( HEADER_TEXTCOLOR == $hamza_lite_header_text_color ) {
		return;
	}
	...
	<?php
		...
		if ( 'blank' == $hamza_lite_header_text_color ) :
	?>
		.site-title,
		.site-description {
			position: absolute;
	...
	?>
		.site-title a,
		.site-description {
			color: #<?php echo $hamza_lite_header_text_color; ?>;
		}
	<?php endif; ?>
	</style>
	<?php
